...
public:
    ...
    std::ofstream generateDefaultContent(winrt::FYP_V2::RecordViewModel& record) {
        ...
        nlohmann::json detailsObj = details.at("detail");

        double cpuTimeSpend = detailsObj.at("CPU Time Spend");
        double gpuTimeSpend = detailsObj.at("GPU Time Spend");
...
public:
	...
	void exportResult() override {
        ...
        std::vector<std::vector<int>> result = detailsObj.at("Result");
