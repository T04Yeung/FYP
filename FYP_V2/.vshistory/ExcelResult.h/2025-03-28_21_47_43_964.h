...
class APSPExcel : public ExcelResult {
public:
    APSPExcel() = default;
...
class filterKruskalExcel : public ExcelResult {
public:
	filterKruskalExcel() = default;
    void exportResult() override {
        winrt::FYP_V2::RecordViewModel record = getRecord();
        ...
        std::vector<std::tuple<int, int, int>> result = details.at("Result");
        winrt::Windows::Foundation::Collections::IVector<winrt::hstring> nodeList = record.NodeList();

        outputFile << "Edge:, Starting Node, Ending Node, Weight\n";

        int totalCost = 0;
        for (auto& edge : result) {
            int n1 = std::get<0>(edge);
            int n2 = std::get<1>(edge);
            int weight = std::get<2>(edge);
            totalCost += weight;

            std::string n1Name = winrt::to_string(nodeList.GetAt(n1));
            std::string n2Name = winrt::to_string(nodeList.GetAt(n2));

            outputFile << " ," << n1Name << "," << n2Name << "," << weight << "\n";
        }
...
class triangleCountingExcel : public ExcelResult {
public:
	triangleCountingExcel() = default;
	void exportResult() override {
        winrt::FYP_V2::RecordViewModel record = getRecord();
		...
		int result = details.at("Result");

        outputFile << "Result:\n";

		outputFile << "Number of Triangle:," << result;
