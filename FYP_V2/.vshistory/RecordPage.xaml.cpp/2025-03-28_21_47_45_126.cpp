...
enum algorithmType {
	ParallelDijkstra, _2DBlockMappedFW, _2DPartitioningBFS, ParallelFilterKruskal, TriangleCounting
};
    ...
    for (auto record = records.rbegin(); record != records.rend();record++) {
		if (filter->filter(*record)) {
			displayRecords.Append(RecordViewModel::Create(
				...
				record->getTimeSpend(),
                ConvertNodeListToNameVector(record->getNodeList()),
...
void winrt::FYP_V2::implementation::RecordPage::ExportBtn_Click(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
{
    ...
    int algorithmIdx = record.AlgorithmIdx();

    if (algorithmIdx == algorithmType::ParallelDijkstra || algorithmIdx == algorithmType::_2DBlockMappedFW) {
        auto er = APSPExcel();
        er.setRecord(record);
        er.exportResult();
	}
    else if (algorithmIdx == algorithmType::_2DPartitioningBFS) {
        auto er = _2DPartitioningBFSExcel();
        er.setRecord(record);
        er.exportResult();
    }
    else if (algorithmIdx == algorithmType::ParallelFilterKruskal) {
		auto er = filterKruskalExcel();
		er.setRecord(record);
		er.exportResult();
	}
	else if (algorithmIdx == algorithmType::TriangleCounting) {
		auto er = TriangleCountingExcel();
		er.setRecord(record);
		er.exportResult();
	}
    ...
    successDialog.Title(winrt::box_value(hstring(L"Message")));
    successDialog.Content(winrt::box_value(hstring(L"Success!")));
    successDialog.PrimaryButtonText(hstring(L"Yes"));

    successDialog.ShowAsync();
}

winrt::Windows::Foundation::Collections::IVector<winrt::hstring> ConvertNodeListToNameVector(std::vector<Node>& nodeList) {
    std::vector<winrt::hstring> result;
    for (auto& node : nodeList) {
        result.push_back(winrt::to_hstring(node.name));
    }
