...
public:
	...
	std::vector<Edge> getUndirectedEdges() {
		...
		for (int i = 0; i < numOfVertices; i++) {
			for (int j = i + 1; j < numOfVertices; j++) {
				...
				int weight = min(tempEdge1, tempEdge2);
	...
	int** getUndirectedAdjacencyMatrix() {
		int** undirectedAdjMatrix = new int* [numOfVertices];
		for (int i = 0; i < numOfVertices; i++) {
			undirectedAdjMatrix[i] = new int[numOfVertices];
		}
		for (int i = 0; i < numOfVertices; i++) {
			for (int j = 0; j < numOfVertices; j++) {
				undirectedAdjMatrix[i][j] = min(adjacencyMatrix[i][j], adjacencyMatrix[j][i]);
			}
		}
