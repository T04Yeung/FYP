...
bool fileFunctions::createRecordFile(Record record) {
    ...
    try {
        checkSystemDirectoryExistence();

        std::stringstream ss;
        ss << std::setw(6) << std::setfill('0') << record.getId();
        std::string idStr = ss.str();

        std::filesystem::path filePath = dataRootDirectoryStr + subDirectoryList[0] + idStr;
...
void checkSystemDirectoryExistence() {
    std::filesystem::path directory = fileFunctions::dataRootDirectoryStr;
    ...
    std::filesystem::path configFile = fileFunctions::dataRootDirectoryStr + "\\config.json";
    ...
    for (std::string subDirectory : fileFunctions::subDirectoryList) {
        std::filesystem::path tempDirectory = fileFunctions::dataRootDirectoryStr + subDirectory;
