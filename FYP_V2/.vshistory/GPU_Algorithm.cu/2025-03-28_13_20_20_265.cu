...
__global__ void filterKruskalKernel(Edge* edges, int numOfEdges, int* parent, int* rank, Edge* mst, int* size, int medianWeight) {
...
std::vector<Edge> GPU_Algorithm::GPU_Parallel_FilterKruskal(Graph& graph) {
    ...
    Edge* d_edges;
    int* d_parent, * d_rank, * d_mstSize, * d_newCount;
    Edge* d_mstEdges;
