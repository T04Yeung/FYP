...
std::vector<Edge> GPU_Algorithm::GPU_Parallel_FilterKruskal(Graph& graph) {
    ...
    thrust::sort(thrust::device, d_edges, d_edges + numOfEdges, [] (Edge & e1, Edge & e2) {
    ...
    while (remainingEdges > 0) {
		...
		auto endOfEdges = thrust::partition(thrust::device, d_edges, d_edges + remainingEdges, [medianWeight] (Edge& e) {
