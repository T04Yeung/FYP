...
std::vector<Edge> GPU_Algorithm::GPU_Parallel_FilterKruskal(Graph& graph) {
	...
	std::vector<Edge> h_edges(edges.size());

    for (auto& edge : edges) {
		h_edges.emplace_back(edge.id, edge.from, edge.to, edge.weight);
    }
    ...
    cudaMemcpy(d_edges, h_edges.data(), numOfEdges * sizeof(Edge), cudaMemcpyHostToDevice);
