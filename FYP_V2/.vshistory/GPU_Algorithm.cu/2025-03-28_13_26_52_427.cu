...
std::vector<Edge> GPU_Algorithm::GPU_Parallel_FilterKruskal(Graph& graph) {
    ...
    thrust::sort(thrust::device, d_edges, d_edges + numOfEdges, [] __device__(Edge & e1, Edge & e2) {
    ...
    while (remainingEdges > 0) {
		...
		auto endOfEdges = thrust::partition(thrust::device, d_edges, d_edges + remainingEdges, [medianWeight] __device__(Edge& e) {
			return e.weight <= medianWeight;
		});
		remainingEdges = endOfEdges - d_edges;
    }
