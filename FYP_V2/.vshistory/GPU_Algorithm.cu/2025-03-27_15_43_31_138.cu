...
__global__ void kosarajuKernel(
    const int* adjRowPtrs,   // CSR row pointers
    const int* adjCols,      // CSR column indices
    ...
    if (atomicCAS(&visited[currentVertex], expected, 1) == 0) {
        ...
        while (front < rear) {
            int u = queue[front++];
            for (int i = adjRowPtrs[u]; i < adjRowPtrs[u + 1]; ++i) {
                int v = adjCols[i];
