...
__global__ void filterKruskalKernel(GPU_Edge* edges, int numOfEdges, int* parent, int* rank, GPU_Edge* mst, int* mstSize, int medianWeight) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= numOfEdges) return;

    GPU_Edge edge = edges[idx];
