...
__global__ void kosarajuKernel(
    ...
    if (atomicCAS(&visited[currentVertex], 0, 1) == 0) {
        int componentId = currentVertex;
        components[currentVertex] = componentId;

        // Use shared memory for queue (adjust size as needed)
        ...
        while (front < rear) {
            ...
            for (int v = 0; v < numVertices; v++) {
                if (adjMatrix[u * numVertices + v] != 0) { // Check reversed edge
                    if (atomicCAS(&visited[v], 0, 1) == 0) {
                        components[v] = componentId;
                        if (rear < 1024) { // Prevent overflow
                            queue[rear++] = v;
                        }
                    }
                }
...
std::vector<std::vector<int>> GPU_Algorithm::GPU_Kosaraju(Graph& graph) {
    ...
    cudaMemset(d_visited, 0, numOfVertices * sizeof(int));
