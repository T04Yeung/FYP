...
__global__ void filterKruskalKernel(Edge* edges, int numOfEdges, int* parent, int* rank, Edge* mst, int* mstSize, int medianWeight, bool* edgeAdded) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (rootA != rootB && unite(rootA, rootB, parent, rank)) {
		bool added = true;
        if (!atomicExch(&edgeAdded[idx], added)) {
...
std::vector<Edge> GPU_Algorithm::GPU_Parallel_FilterKruskal(Graph& graph) {
    ...
    cudaMalloc(&d_edgeAdded, numOfEdges * sizeof(bool));
    ...
    cudaMemset(d_edgeAdded, 0, numOfEdges * sizeof(bool));
