...
__global__ void kosarajuKernel(
    ...
    // Try to claim the node if unvisited
    bool expected = false;
    if (atomicCAS(&visited[currentVertex], expected, true)) {
        // Assign component ID (smallest vertex in the SCC)
        int componentId = currentVertex;
        components[currentVertex] = componentId;

        // BFS to propagate component ID
        int* queue = new int[numVertices]; // Simplified for demonstration
        int front = 0, rear = 0;
        ...
        while (front < rear) {
            ...
            for (int v = 0; v < numVertices; ++v) {
                if (adjMatrix[u * numVertices + v] && !visited[v]) {
                    if (atomicCAS(&visited[v], false, true)) {
