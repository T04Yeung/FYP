...
std::vector<Edge> GPU_Algorithm::GPU_Parallel_FilterKruskal(Graph& graph) {
    ...
    while (remainingEdges > 0) {
        ...
        auto weightEnd = thrust::partition(thrust::device, d_edges, d_edges + remainingEdges,
            [medianWeight] __host__ __device__(const Edge & e) {
            return e.weight <= medianWeight;
        });
        int numProcessed = weightEnd - d_edges;
        ...
        remainingEdges = (newEnd - d_edges) + (d_edges + remainingEdges - weightEnd);
        thrust::merge(thrust::device,
            d_edges, newEnd,
            weightEnd, d_edges + remainingEdges,
            d_edges);
    }
