...
std::vector<Edge> GPU_Algorithm::GPU_Parallel_FilterKruskal(Graph& graph) {
    ...
    while (remainingEdges > 0) {
        ...
        auto endOfEdges = thrust::partition(thrust::device, d_edges, d_edges + remainingEdges, [medianWeight] __host__ __device__(const Edge & e) {
...
int GPU_Algorithm::GPU_Triangle_Counting(const Graph& graph) {
	int numOfVertices = graph.getNumOfVertices();
	int** adjMatrix = graph.getAdjacencyMatrix();
    ...
    for (int i = 0; i < numOfVertices; i++) {
        for (int j = 0; j < numOfVertices; j++) {
            h_A[i * numOfVertices + j] = (adjMatrix[i][j] != 0) ? 1 : 0;
        }
    }
    ...
    matrixMultiplyKernel <<<grid, block>>> (d_A, d_A, d_A2, numOfVertices);
    matrixMultiplyKernel <<<grid, block>>> (d_A2, d_A, d_A3, numOfVertices);

    int h_count;
    cudaMemcpy(d_count, &h_count, sizeof(int), cudaMemcpyHostToDevice);
    traceKernel <<<(numOfVertices + 255) / 256, 256 >>> (d_A3, d_trace, numOfVertices);
