...
std::vector<winrt::Windows::Foundation::Point> generateNodePositions() {
    ...
    for (int i = 0; i < numOfVertices; ++i) {
        ...
        do {
            x = distribution(gen);
            y = distribution(gen);
        } while (unique_positions.count({ x, y }));
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int j = 0; j < numOfVertices; j++) {
            double dispLength = std::hypot(disp[j].X, disp[j].Y);
            if (dispLength < 1e-5) dispLength = 1e-5;

            nodePositions[j].X += static_cast<float>(
                (disp[j].X / dispLength) * :min(dispLength, currentTemp)
                );
            nodePositions[j].Y += static_cast<float>(
                (disp[j].Y / dispLength) * min(dispLength, currentTemp)
                );
...
void winrt::FYP_V2::implementation::HomePage::visualizeGraph()
{
    ...
    for (int i = 0; i < numOfVertices; ++i) {
        for (int j = i + 1; j < numOfVertices; ++j) {
            if (adjMatrix[i][j] != 0) {
				double x1 = nodePositions[i].X;
                double y1 = nodePositions[i].Y;
                double x2 = nodePositions[j].X;
                double y2 = nodePositions[j].Y;
