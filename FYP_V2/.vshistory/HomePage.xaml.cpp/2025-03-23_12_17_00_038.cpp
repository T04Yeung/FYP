    ...
    for (int i = 0; i < iterations; i++) {
        ...
        for (int j = 0; j < numOfVertices; j++) {
            for (int k = 0; k < numOfVertices; k++) {
                if (j != k) {
                    double dx = disp[j].X - disp[i].X;
                    double dy = disp[j].Y - disp[i].Y;
        ...
        for (int j = 0; i < numOfVertices; i++) {
            ...
            disp[i].X += (disp[i].X / dispLength) * min(dispLength, currentTemp);
            disp[i].Y += (disp[i].Y / dispLength) * min(dispLength, currentTemp);

            disp[i].X = std::clamp(disp[i].X, 100.0f, 4900.0f);
            disp[i].Y = std::clamp(disp[i].Y, 100.0f, 4900.0f);
        }
