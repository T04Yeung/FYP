    ...
    for (int i = 0; i < numOfVertices; ++i) {
        for (int j = i + 1; j < numOfVertices; ++j) {
            if (adjMatrix[i][j] != 0) {
                ...
                winrt::Windows::Foundation::Point tip(adjustedX2, adjustedY2);
                winrt::Windows::Foundation::Point left(
                    tip.X - unitX * arrowSize - unitY * arrowSize,
                    tip.Y - unitY * arrowSize + unitX * arrowSize
                );
                winrt::Windows::Foundation::Point right(
                    tip.X - unitX * arrowSize + unitY * arrowSize,
                    tip.Y - unitY * arrowSize - unitX * arrowSize
                );

                PathFigure arrowFigure;
                arrowFigure.StartPoint(tip);
                //arrowFigure.Segments().Append(LineSegment{ left });
                //arrowFigure.Segments().Append(LineSegment{ right });
