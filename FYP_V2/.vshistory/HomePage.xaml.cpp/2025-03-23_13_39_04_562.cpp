...
void winrt::FYP_V2::implementation::HomePage::visualizeGraph()
{
    ...
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(100.0, canvasSize - 100.0);
    for (int i = 0; i < numOfVertices; ++i) {
        nodePositions.emplace_back(dis(gen), dis(gen));
    }

    const int iterations = 150;
    const double rep = 1500.0;
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        ...
        for (int i = 0; i < numOfVertices; ++i) {
            ...
            nodePositions[i].first = std::clamp(nodePositions[i].first, 50.0, canvasSize - 50.0);
            nodePositions[i].second = std::clamp(nodePositions[i].second, 50.0, canvasSize - 50.0);
        }
    }
...
void winrt::FYP_V2::implementation::HomePage::Page_Unloaded(winrt::Windows::Foundation::IInspectable const&, winrt::Microsoft::UI::Xaml::RoutedEventArgs const&)
{
    auto& sharedData = SharedData::getInstance();
    sharedData.graphZoomFactor = GraphScrollViewer().ZoomFactor();
    sharedData.graphScrollHorizontal = GraphScrollViewer().HorizontalOffset();
    sharedData.graphScrollVertical = GraphScrollViewer().VerticalOffset();
}
