...
void winrt::FYP_V2::implementation::HomePage::visualizeGraph()
{
    ...
    const double centerX = GraphCanvas().Width() / 2;
    const double centerY = GraphCanvas().Height() / 2;
    const double radius = GraphCanvas().Height() / 2 - 50;
    ...
    for (int i = 0; i < numOfVertices; ++i) {
        for (int j = i + 1; j < numOfVertices; ++j) {
            if (adjMatrix[i][j] != 0) {
                ...
                line.X2(x2);
                line.Y2(y2);
                line.Stroke(Media::SolidColorBrush::SolidColorBrush(Colors::Black()));
    ...
    for (int i = 0; i < numOfVertices; ++i) {
        ...
        ellipse.Width(40);
        ellipse.Height(40);
        ellipse.Fill(Media::SolidColorBrush::SolidColorBrush(Colors::Blue()));
        ...
        text.Foreground(Media::SolidColorBrush::SolidColorBrush(Colors::Red()));
