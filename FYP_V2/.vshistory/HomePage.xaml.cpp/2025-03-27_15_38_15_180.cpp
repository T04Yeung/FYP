...
void winrt::FYP_V2::implementation::HomePage::executionBtn_Click(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
{
    ...
    if (deviceIdx == 0) {
        ...
        switch (algorithmIdx) {
            ...
            case 2: {
				std::vector<std::vector<int>> result = CPU_Algorithm::CPU_Kosaraju(graph);
				details["Result"] = result;
				break;
            }
        }
        ...
        switch (algorithmIdx) {
            ...
            case 2: {
                std::vector<std::vector<int>> result = GPU_Algorithm::GPU_Kosaraju(graph);
                details["Result"] = result;
                break;
            }
        }
        ...
        switch (algorithmIdx) {
            ...
            case 2: {
                ...
                std::vector<std::vector<int>> result1 = CPU_Algorithm::CPU_Kosaraju(graph);
                ...
                std::vector<std::vector<int>> result2 = GPU_Algorithm::GPU_Kosaraju(graph);
                details["Result"] = result2;
...
void winrt::FYP_V2::implementation::HomePage::algorithmBox_SelectionChanged(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::Controls::SelectionChangedEventArgs const& e)
{
    int algorithmIndex = algorithmBox().SelectedIndex();

    if (algorithmIndex == 0) {
        dijkstraDetail().Visibility(Visibility::Visible);
        _2dPartitioningDetail().Visibility(Visibility::Collapsed);
    }
    else if (algorithmIndex == 1) {
        dijkstraDetail().Visibility(Visibility::Collapsed);
        _2dPartitioningDetail().Visibility(Visibility::Visible);
    }
