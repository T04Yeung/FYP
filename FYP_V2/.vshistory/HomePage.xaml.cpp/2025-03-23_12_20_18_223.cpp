...
void winrt::FYP_V2::implementation::HomePage::visualizeGraph()
{
    ...
    const Shapes::Rect visibleArea = GetVisibleArea(GraphScrollViewer());
    const bool drawDetails = zoomFactor > 0.5;
    const bool drawArrows = zoomFactor > 0.8;
    const bool drawLabels = zoomFactor > 1.0;
...
winrt::Windows::Foundation::Rect GetVisibleArea(const ScrollViewer& scrollViewer) {
    const double zoom = scrollViewer.ZoomFactor();
    return {
        scrollViewer.HorizontalOffset() / zoom,
        scrollViewer.VerticalOffset() / zoom,
        scrollViewer.ViewportWidth() / zoom,
        scrollViewer.ViewportHeight() / zoom
    };
}

std::vector<winrt::Windows::Foundation::Point> winrt::FYP_V2::implementation::HomePage::generateNodePositions() {
	...
	std::mt19937 gen(rd());
	std::uniform_real_distribution<double> distribution(100.0, 4900.0);
	for (int i = 0; i < numOfVertices; ++i) {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        for (int j = 0; i < numOfVertices; i++) {
            double dispLength = max(1.0, std::hypot(disp[i].X, disp[i].Y));
            nodePositions[i].X += (disp[i].X / dispLength) * min(dispLength, currentTemp);
            nodePositions[i].Y += (disp[i].Y / dispLength) * min(dispLength, currentTemp);

            nodePositions[i].X = std::clamp(disp[i].X, 100.0f, 4900.0f);
            nodePositions[i].Y = std::clamp(disp[i].Y, 100.0f, 4900.0f);
        }
