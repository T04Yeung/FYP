...
const std::vector<std::string> algorithmNameList = {
    ...
    "2D Block-Mapped Floyd-Warshall algorithm",
    "2D Partitioning BFS",
	"Parallel Filter Kruskal's algorithm",
...
void winrt::FYP_V2::implementation::HomePage::ImportVertices(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
{
    ...
    if (fileFunctions::isCSVFile(filename)) {
        ...
        if (currentNodes.size() > 500) {
            ShowMessage(L"Because of the performance issue, the graph can only display at most 500 nodes.\nYou graph will not be display on the panel.");
        }
...
void winrt::FYP_V2::implementation::HomePage::ImportEdges(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
{
    ...
    if (fileFunctions::isCSVFile(filename)) {
        ...
        getNodesPosition(graph.getAdjacencyMatrix(), graph.getNumOfVertices());
...
{
    if (currentNodes.empty() || currentEdges.empty()) {
		ShowMessage(L"Please import graph first!");
		return;
    }
    ...
    if (deviceIdx == 0) {
        ...
        switch (algorithmIdx) {
            ...
            case 1: {
                std::vector<std::vector<int>> result = CPU_Algorithm::CPU_2DBlockMappedFW(graph);
                details["Result"] = result;
                break;
            }
            case 2: {
            ...
            case 3: {
				std::vector<Edge> result = CPU_Algorithm::CPU_Parallel_FilterKruskal(graph);
                std::vector<std::tuple<int, int, int>> tempResult;

				for (auto& edge : result) {
					tempResult.emplace_back(edge.from, edge.to, edge.weight);
				}

                details["Result"] = tempResult;
                break;
            }
            case 4: {
                int result = CPU_Algorithm::CPU_Triangle_Counting(graph);
                details["Result"] = result;
                break;
            }
        }
        ...
        switch (algorithmIdx) {
            ...
            case 1: {
                std::vector<std::vector<int>> result = GPU_Algorithm::GPU_2DBlockMappedFW(graph);
                details["Result"] = result;
                break;
            }
            case 2: {
            ...
            case 3: {
                std::vector<Edge> result = GPU_Algorithm::GPU_Parallel_FilterKruskal(graph);
                std::vector<std::tuple<int, int, int>> tempResult;
            ...
            case 4: {
				int result = GPU_Algorithm::GPU_Triangle_Counting(graph);
                details["Result"] = result;
                break;
            }
        }
        ...
        switch (algorithmIdx) {
            case 0: {
                ...
                CPU_Algorithm::CPU_Parallel_Dijkstra(graph);
            ...
            case 1: {
                ...
                CPU_Algorithm::CPU_2DBlockMappedFW(graph);
            ...
            case 2: {
                ...
                CPU_Algorithm::CPU_2DPartition_BFS(graph, 0);
            ...
            case 3: {
                ...
                CPU_Algorithm::CPU_Parallel_FilterKruskal(graph);
            ...
            case 4: {
                ...
                CPU_Algorithm::CPU_Triangle_Counting(graph);
    ...
    Record record = Record(recordManagement::getLatestRecordId(), time(0), deviceIdx, deviceNameList.at(deviceIdx),
        algorithmIdx, algorithmNameList.at(algorithmIdx), timeSpend, currentNodes, details);

    fileFunctions::createRecordFile(record);

    ShowMessage(L"Success! Check the result in Record Page.");
}

void winrt::FYP_V2::implementation::HomePage::visualizeGraph()
{
    int numOfVertices = graph.getNumOfVertices();
    if (numOfVertices == 0 || numOfVertices > 500) {
        return;
    }
...
{
	auto iter = SharedData::getInstance().snapshotsEdges.find(SharedData::getInstance().currentDisplayGraphId);
	if (iter == SharedData::getInstance().snapshotsEdges.begin()) {
		return;
	}
	iter--;
	SharedData::getInstance().currentDisplayGraphId = iter->first;
	graph = SharedData::getInstance().getGraph(SharedData::getInstance().currentDisplayGraphId);
    getNodesPosition(graph.getAdjacencyMatrix(), graph.getNumOfVertices());
	visualizeGraph();
}
...
{
	SharedData::getInstance().removeSnapshot(SharedData::getInstance().currentDisplayGraphId);
	graph = SharedData::getInstance().getGraph(SharedData::getInstance().currentDisplayGraphId);
    getNodesPosition(graph.getAdjacencyMatrix(), graph.getNumOfVertices());
	visualizeGraph();
}
...
{
	auto iter = SharedData::getInstance().snapshotsEdges.find(SharedData::getInstance().currentDisplayGraphId);
	if (++iter == SharedData::getInstance().snapshotsEdges.end()) {
		return;
	}
    SharedData::getInstance().currentDisplayGraphId = iter->first;
    graph = SharedData::getInstance().getGraph(SharedData::getInstance().currentDisplayGraphId);
