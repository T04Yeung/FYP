...
{
	auto iter = SharedData::getInstance().deltas.find(SharedData::getInstance().currentDisplayGraphId);
	if (iter == SharedData::getInstance().deltas.begin()) {
		return;
	}
	iter--;
...
{
	auto iter = SharedData::getInstance().deltas.find(SharedData::getInstance().currentDisplayGraphId);
	if (++iter == SharedData::getInstance().deltas.end()) {
		return;
	}
    SharedData::getInstance().currentDisplayGraphId = iter->first;
    graph = SharedData::getInstance().getGraph(SharedData::getInstance().currentDisplayGraphId);
