...
void winrt::FYP_V2::implementation::HomePage::executionBtn_Click(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
{
    ...
    if (deviceIdx == 0) {
        ...
        timeStr = "CPU - Time:" + duration.count();
        ShowMessage(winrt::to_hstring(duration.count()));
    }
    else if (deviceIdx == 1) {
        auto start = std::chrono::high_resolution_clock::now();
        switch (algorithmIdx) {
        ...
        timeStr = "GPU - Time:" + duration.count();
    }
    else if (deviceIdx == 2) {
        ...
        switch (algorithmIdx) {
            case 0: {
                ...
                timeStr += "CPU - Time:" + duration1.count();
                timeStr += "\n";
                ...
                timeStr += "GPU - Time:" + duration1.count();
                break;
            }
            case 1: {
                auto start1 = std::chrono::high_resolution_clock::now();
    ...
    //ShowMessage(winrt::to_hstring(timeStr));
}
