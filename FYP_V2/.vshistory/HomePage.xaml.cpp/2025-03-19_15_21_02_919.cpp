...
void winrt::FYP_V2::implementation::HomePage::executionBtn_Click(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
{
    ...
    if (deviceIdx == 0) {
        ...
        graphText().Text(winrt::to_hstring(duration.count() + "μs"));
    }
    else if (deviceIdx == 1) {
        auto start = std::chrono::high_resolution_clock::now();
        switch (algorithmIdx) {
        ...
        switch (algorithmIdx) {
            case 0: {
                ...
                graphText().Text(winrt::to_hstring(duration1.count() + duration2.count()));
                break;
            }
            case 1: {
                auto start1 = std::chrono::high_resolution_clock::now();
                ...
                graphText().Text(winrt::to_hstring((duration1.count() + duration2.count()) + "μs"));
                break;
            }
        }
    }
