...
std::vector<winrt::Windows::Foundation::Point> generateNodePositions() {
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int j = 0; j < numOfVertices; j++) {
            double dispLength = max(1.0, std::hypot(disp[j].X, disp[j].Y));
            nodePositions[j].X += static_cast<float>((disp[j].X / dispLength) * std::min(dispLength, currentTemp));
            nodePositions[j].Y += static_cast<float>((disp[j].Y / dispLength) * std::min(dispLength, currentTemp));
