		...
		for (int i = 0; i < numOfBlocks; i++) {
			for (int j = 0; j < numOfBlocks; j++) {
				...
				if (!subMatrix[i][j].empty()) {
					std::queue<int> currentQueue = std::move(subMatrix[i][j]); // Take all elements
					subMatrix[i][j] = std::queue<int>(); // Clear the original queue
					omp_unset_lock(&matrixLock[i][j]);

					while (!currentQueue.empty()) {
						int currentVertex = currentQueue.front();
						currentQueue.pop();
...
std::vector<Edge> FilterKruskal_Recursion(std::vector<Edge>& edges, UnionFind& unionFind, int threshold, int size) {
	...
	{
		UnionFind localUnionFind = UnionFind(size);
		leftMST = FilterKruskal_Recursion(leftEdges, unionFind, threshold, size);
	}
	#pragma omp taskwait

	for (const Edge& e : leftMST) {
	...
	std::vector<Edge> rightMST = FilterKruskal_Recursion(filteredRight, unionFind, threshold, size);
...
std::vector<Edge> CPU_Algorithm::CPU_Parallel_FilterKruskal(Graph& graph) {
	...
	return FilterKruskal_Recursion(edges, unionFind, threshold, graph.getNumOfVertices());
}

int CPU_Algorithm::CPU_Triangle_Counting(Graph& graph) {
	int numOfVertices = graph.getNumOfVertices();
	int** adjMatrix = graph.getAdjacencyMatrix();

	std::vector<int> A(numOfVertices * numOfVertices);
	for (int i = 0; i < numOfVertices; i++) {
		for (int j = 0; j < numOfVertices; j++) {
			A[i * numOfVertices + j] = (adjMatrix[i][j] != 0 || adjMatrix[j][i] != 0) ? 1 : 0;
		}
	}

	std::vector<int> A2(numOfVertices * numOfVertices, 0), A3(numOfVertices * numOfVertices, 0);

	#pragma omp parallel for collapse(2)
	for (int i = 0; i < numOfVertices; i++) {
		for (int j = 0; j < numOfVertices; j++) {
			for (int k = 0; k < numOfVertices; k++) {
				A2[i * numOfVertices + j] += A[i * numOfVertices + k] * A[k * numOfVertices + j];
			}
		}
	}
	...
	for (int i = 0; i < numOfVertices; i++) {
		for (int j = 0; j < numOfVertices; j++) {
			for (int k = 0; k < numOfVertices; k++) {
				A3[i * numOfVertices + j] += A2[i * numOfVertices + k] * A[k * numOfVertices + j];
			}
		}
	}
