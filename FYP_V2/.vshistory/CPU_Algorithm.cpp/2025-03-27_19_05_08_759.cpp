...
std::vector<std::vector<int>> CPU_2DBlockMappedFW(const Graph& graph) {
	int numOfVertices = graph.getNumOfVertices();
	int** adjMatrix = graph.getAdjacencyMatrix();
	...
	for (int k = 0; k < numOfVertices; k++) {
		int blockIdx_x = (numOfVertices + tile_size - 1) / tile_size;
		int blockIdx_y = (numOfVertices + tile_size - 1) / tile_size;

		#pragma omp parallel for collapse(2)
		for (int y = 0; y < blockIdx_y; y++) {
			for (int x = 0; x < blockIdx_x; x++) {

				int i_start = y * tile_size;
				int i_end = min(i_start + tile_size, numOfVertices);
				int j_start = x * tile_size;
				int j_end = min(j_start + tile_size, numOfVertices);
