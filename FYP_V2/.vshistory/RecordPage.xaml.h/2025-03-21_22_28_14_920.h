...
private:
    ...
    RecordFilter() : dateFrom(), dateTo(), deviceIdx(0), algorithmIdx(0) {}
...
public:
    ...
    bool filter(Record& record) {
		time_t recordDateStr = record.getExecutionDate();

        if (dateFrom != NULL && dateFrom > recordDateStr) {
			return false;
        }

        if (dateTo != NULL && dateFrom <= recordDateStr) {
            return false;
        }

        if (deviceIdx != 0 && deviceIdx == record.getDeviceIdx()) {
            return false;
        }

        if (algorithmIdx != 0 && algorithmIdx == record.getAlgorithmIdx()) {
            return false;
        }
