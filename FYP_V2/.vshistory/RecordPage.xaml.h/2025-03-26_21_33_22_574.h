...
winrt::param::vector<winrt::hstring> ConvertNodeListToHStringVector(const std::vector<Node>& nodeList) {
    std::vector<winrt::hstring> hstringVector;
    for (const auto& node : nodeList) {
        hstringVector.push_back(winrt::to_hstring(node.name));
    }
    return winrt::param::vector<winrt::hstring>(std::move(hstringVector));
}
        ...
        RecordPage()
        {
            ...
            for (auto record = records.rbegin(); record != records.rend(); record++) {
                displayRecords.Append(RecordViewModel::Create(
                    ...
                    record->getTimeSpend(),
                    ConvertNodeListToHStringVector(record->getNodeList()),
