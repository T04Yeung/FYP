    ...
    struct RecordPage : RecordPageT<RecordPage>
    {
        RecordPage()
        {
            ...
            for (auto& record : records) {
                displayRecords.Append(winrt::make<FYP_V2::RecordViewModel>(record.getId(), record.getExecutionDateStr(), record.getDeviceIdx(), record.getDeviceName(), record.getAlgorithmIdx(), 
                    record.getAlgorithmName(), record.getTimeSpend(), record.getDetails()));
            }
        }
