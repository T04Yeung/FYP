...
void fileFunctions::checkSystemDirectoryExistence() {
	checkSystemDirectoryExistence(dataRootDirectoryStr);
}

void fileFunctions::checkSystemDirectoryExistence(std::string directoryStr) {
    ...
    std::filesystem::path configFile = directoryStr + "\\config.json";
    ...
    for (std::string subDirectory : subDirectoryList) {
        std::filesystem::path tempDirectory = directoryStr + subDirectory;
...
bool fileFunctions::createRecordFile(Record record) {
    ...
    recordJson["details"] = record.getDetails();
...
std::vector<Record> fileFunctions::getAllRecords() {
    ...
    for (const auto& record : std::filesystem::directory_iterator(directoryPath)) {
        if (record.is_regular_file() && record.path().extension() == ".json") {
            ...
            std::vector<Node> nodeList = recordJson["nodeList"].get<std::vector<Node>>();
            ...
            records.emplace_back(
                ...
                algorithmIdx, algorithmName, timeSpend, nodeList, details
            );
        }
    }
